...
public class University implements UniversityInt {
	...
	private void TimeSimulate() {
		...
		if (Config.testMode == false) {
			...
			}, (20000 * 34));// System.out.println("test");
	...
	public Student GetStudent(int number) {
		...
		for (int i = 0; i < students.size(); i++) {
			if (students.get(i).StudentNumber() == number) {
				...
				logger.info(
						String.format("University Operation: Get student; Student info: %d; State: Success", number));
			}
		}
		return student;
	}
	...
	public boolean LookupStudent(int number, String name) {
		...
		if (flag != 0) {
			result = true;
			logger.info(String.format("University Operation: Lookup student; Student info: [%d,%s]; State: Success",
					number, name));
		} else {
			result = false;
			logger.info(String.format("University Operation: Lookup student; Student info: [%d,%s]; State: Fail",
					number, name));
		}
		return result;
	}
	...
	@Override
	public boolean CreateCourse(String title, int mycode, int cap, boolean enforcePrereqs, int numberofmidterms,
			int numberofassignments, boolean hasafinal, boolean isprojectcourse) {
		// TODO Auto-generated method stub
		boolean result = true;
		...
		if (!CheckCourse(mycode)) {
			if (!isprojectcourse) {
				Course c = new Course(title, mycode, cap, enforcePrereqs, numberofmidterms, numberofassignments,
						hasafinal);
				result = courses.add(c);
				logger.info(
						String.format("University Operation: Create new course; Course info: [%s,%d]; State: Success",
								title, mycode));
			} else {
				ProjectCourse c = new ProjectCourse(title, mycode, cap, enforcePrereqs, numberofmidterms,
						numberofassignments, hasafinal);
				result = courses.add(c);
				logger.info(String.format(
						"University Operation: Create new project course; Course info: [%s,%d]; State: Success", title,
						mycode));
			}
		} else {
			result = false;
			logger.info(String.format(
					"University Operation: Create new course; Course info: [%s,%d]; State: Fail; Reason: The course already existed.",
					title, mycode));
		}
		return result;
	}
	...
	@Override
	public boolean CreateStudent(int number, String name, boolean isfulltime) {
		// TODO Auto-generated method stub
		boolean result = true;
		...
		if (!CheckStudent(number)) {
			...
			logger.info(String.format("University Operation: Create new student; Student info: [%d,%s]; State: Success",
					number, name));
		} else {
			result = false;
			logger.info(String.format(
					"University Operation: Create new student; Student info: [%d,%s]; State: Fail; Reason: The student already existed.",
					number, name));
		}
		return result;
	}
	...
	public boolean DeRegisterStudentFromCourse(Student student, Course course) {
		...
		if (CheckCourse(course.Code()) && CheckStudent(student.StudentNumber()) && student.IsRegistered(course)) {
			...
			if (result) {
				...
				logger.info(String.format("University Operation: student %d deregister course %d; State: Fail",
						student.StudentNumber(), course.Code()));
			}
		} else {
			result = false;
			logger.info(String.format(
					"University Operation: student %d deregister course %d; State: Fail; Reason: The student or course doesn't exist or the student hasn't registered the course.",
					student.StudentNumber(), course.Code()));
		}
		return result;
	}
	...
	public boolean CancelCourse(Course course) {
		...
		if (CheckCourse(course.Code())) {
			...
			result = false;
			logger.info(String.format(
					"University Operation: cancel course %d; State: Fail; Reason: The course doesn't exist.",
					course.Code()));
		}
		return result;
	}
...
//			List<Student> s = new ArrayList<Student>();
			...
			result = false;
			logger.info(String.format(
					"University Operation: delete course %d; State: Fail; Reason: The course doesn't exist.",
					course.Code()));
		}
		return result;
	}
...
//			List<Course> c = new ArrayList<Course>();
			...
			logger.info(
					String.format("University Operation: delete student %d; State: Success", student.StudentNumber()));
		} else {
			result = false;
			logger.info(String.format(
					"University Operation: delete student %d; State: Fail; Reason: The student doesn't exist.",
	...
	public boolean MarkStudents(Course course) {
		...
		if (CheckCourse(course.Code())) {
			if (!course.getEnrollStudent().isEmpty()) {
				...
				result = true;
				logger.info(String.format("University Operation: mark students for course %d; State: Success",
						course.Code()));
			} else {
				result = false;
				logger.info(String.format(
						"University Operation: mark students for course %d; State: Fail Reason: No students enrolled in this course.",
						course.Code()));
			}
		} else {
			result = false;
			logger.info(String.format(
					"University Operation: mark students for course %d; State: Fail; Reason: The course doesn't exist.",
					course.Code()));
		}
		return result;
	}
	...
	public int DoAssignments(Student student, Course course) {
		...
		if (cantakecourse) {
			...
			if (weight > 0) {
				weight = weight / 100;
				mark = course.getEnrollStudent().get(student) + weight * (random.nextInt(25) + 75);
